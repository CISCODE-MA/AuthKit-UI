...
export const InlineError: React.FC<Props> = ({ message, dismissAfterMs = 2000000 }) => {
  ...
  return (
    ...
    >
      {/* Left accent bar */}
      <span className="absolute left-0 top-0 h-full w-1 bg-gradient-to-b from-red-500 to-red-400" />

      {/* Icon */}
      <AlertTriangle className="mt-0.5 h-5 w-5 flex-none" />

      {/* Message */}
      <span className="grow leading-5">{message}</span>

      {/* Dismiss button */}
      <button
        onClick={() => setShow(false)}
        className="absolute right-2 top-2 rounded p-1 text-red-600/70
